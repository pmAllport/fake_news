...
class Un_freezed():
    ...
    def train_model(self,weight_matrix,length,train_data,train_values,test_data,test_values):
        ...
        model = tf.keras.Sequential([
            ...
            tf.keras.layers.Conv1D(12, 8, activation='sigmoid'),
            tf.keras.layers.MaxPooling1D(pool_size=8),
            tf.keras.layers.Bidirectional(LSTM(10,recurrent_dropout=(0.3), return_sequences=True)),
        ...
        history = model.fit(train_data, train_values, batch_size=7, epochs=12, shuffle = True,validation_data=(test_data, test_values)) 

        model.save(str(os.getcwd())+"/"+"models"+'LSTM_CNN_UnFreezed.h5', model)
...
if __name__ == '__main__':
    un_freezed = Un_freezed('en')
    un_freezed.main(True)        
...
""" def train_model(self,weight_matrix,length,train_data,train_values,test_data,test_values,epochs,dp1,dp2,dp3,bsize,rdrop,dense1,lstm1):
        ...
        model = tf.keras.Sequential([
            ...
            tf.keras.layers.Bidirectional(LSTM(lstm1,recurrent_dropout=(rdrop))),
